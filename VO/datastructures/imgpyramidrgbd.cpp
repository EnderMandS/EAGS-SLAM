...
ImgPyramidRGBD::ImgPyramidRGBD(const ImgPyramidSettings &settings,
    ...
    : cameraPyr(cameraPyr), timeStamp(0), mSettings(settings),
      mIsStored(false) {
  I3D_LOG(i3d::debug) << "ImgPyramidRGBD construct.";
}

ImgPyramidRGBD::~ImgPyramidRGBD() {
  I3D_LOG(i3d::debug) << "~ImgPyramidRGBD()";
  // for (auto p:optimizationStructure) {
...
ImgPyramidRGBD::ImgPyramidRGBD(const ImgPyramidSettings &settings,
    ...
    : cameraPyr(cameraPyr), timeStamp(timestamp), mSettings(settings),
      mIsStored(false) {
  I3D_LOG(i3d::debug) << "Creating ImgPyramidRGBD.";
  ...
  cv::Mat gray;
  cv::cvtColor(rgbFullSize, gray, CV_BGR2GRAY);
...
float ImgPyramidRGBD::generateDistHistogram(const cv::Mat &edges,
  ...
  for (int yy = 0; yy < edges.rows; ++yy) {
    for (int xx = 0; xx < edges.cols; ++xx) {
      if (edges.at<u_int8_t>(yy, xx) > 0)
        dist.at<u_int8_t>(floor((float)yy / PATCH_SIZE),
                          floor((float)xx / PATCH_SIZE))++;
    }
  }
  ...
  I3D_LOG(i3d::detail) << "nDist: " << nDist;
...
void ImgPyramidRGBD::makeKeyframe() {
  ...
  for (int lvl = 0; lvl < mSettings.nLevels(); ++lvl) {
    ...
    std::shared_ptr<Eigen::Vector4f> optStruc(
        static_cast<Eigen::Vector4f *>(Eigen::internal::aligned_malloc(
            int(cam.area) * sizeof(Eigen::Vector4f))),
...
void ImgPyramidRGBD::buildOptimizationStructure(
    const cv::Mat &floatImg, std::shared_ptr<Eigen::Vector4f> startPt) const {
  ...
  Eigen::Vector4f *gradxyii_pt = startPt.get() + width; // distGradients + width;
