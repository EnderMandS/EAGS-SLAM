...
public:
  ...
  std::shared_ptr<Eigen::Vector4f> returnOptimizationStructure(uint lvl) const {
    assert(lvl >= 0 && lvl < optimizationStructure.size());

    if (optimizationStructureBuilt.at(lvl))
      return optimizationStructure.at(lvl);
...
public:
  ...
  const inline Eigen::Matrix4f getT_w_f() const { return T_w_f; }
...
private:
  ...
  void
  buildOptimizationStructure(const cv::Mat &floatImg,
                             std::shared_ptr<Eigen::Vector4f> startPt) const;
  ...
  // the pcl of edges
  std::vector<Eigen::MatrixXf, Eigen::aligned_allocator<Eigen::MatrixXf>> edges3DPyr;
  ...
  std::vector<std::shared_ptr<Eigen::Vector4f>> optimizationStructure;
