...
TrackerNew::TrackerStatus TrackerNew::assessTrackingQuality(
  ...
  I3D_LOG(i3d::debug) << "Assessing tracking quality.";
  ...
  float overlap_ratio = overlapMeasure / overlaps.at(0);
  I3D_LOG(i3d::detail) << "Sum(wHi): " << overlapMeasure
                     << ", H(0): " << overlaps.at(0)
                     << ", ratio: " << overlap_ratio;
  if (overlapMeasure >= overlaps.at(0) || histogram.size() < 4) {
    // || overlaps.at(histogram.size()-1) > overlaps.at(1)*0.5)

    // output reprojections
    // cv::imwrite("out/M_" + std::to_string(mCurrFrame->frameId) + "_no.png",
    // M); cv::imwrite("out/currEdges" + std::to_string(mCurrFrame->frameId) +
    //                 "_no.png",
    //             currEdges);
    // cv::imwrite("out/currDepth" + std::to_string(mCurrFrame->frameId) +
    //                 "_no.png",
    //             currDepth);
    I3D_LOG(i3d::debug) << "No Need to add new keyframe.";
    return TRACKER_STATE_OK;
  }

  // output reprojections
  // cv::imwrite("out/M_" + std::to_string(mCurrFrame->frameId) + "_new.png",
  // M); cv::imwrite("out/currEdges" + std::to_string(mCurrFrame->frameId) +
  //                 "_new.png",
  //             currEdges);
  // cv::imwrite("out/currDepth" + std::to_string(mCurrFrame->frameId) +
  //                 "_new.png",
  //             currDepth);
  I3D_LOG(i3d::debug) << "Need to add new keyframe.";
...
void TrackerNew::checkInitializationValues(
  ...
  if (costEye < costInit) {
    I3D_LOG(i3d::debug) << "DO NOT INIT WITH PREVIOUS TRANSFORM!" << costEye
                        << " " << costInit;
    R.setIdentity();
    T.setZero();
  }
...
TrackerNew::trackFrames(Eigen::Matrix3f &R, Eigen::Vector3f &T, float &error,
  ...
  I3D_LOG(i3d::debug) << "Tracking at frame: " << currFrame->frameId;
  ...
  I3D_LOG(i3d::debug) << "checkInitializationValues: "
                      << Timer::getTimeDiffMiS(beginInit, endInit) << "us";
  error = INFINITY;
  ...
  for (int lvl = mPyrConfig.PYR_MIN_LVL; lvl >= mPyrConfig.PYR_MAX_LVL; --lvl) {
    I3D_LOG(i3d::detail)
        << "----LEVEL----"
        << lvl; // << " curr: " << std::fixed << currFrame->getTimestamp() <<
                // " to " << refFrame->getTimestamp();
    I3D_LOG(i3d::detail) << "\nR init:\n" << R;
    I3D_LOG(i3d::detail) << "\nt init:\n" << T.transpose();
    // perform tracking
  ...
  I3D_LOG(i3d::detail) << "FINAL AVG ERROR=" << error
