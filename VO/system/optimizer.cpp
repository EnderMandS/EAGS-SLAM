...
double
Optimizer::calcErrorAndBuffers(const std::shared_ptr<ImgPyramidRGBD> &refFrame,
                               const std::shared_ptr<ImgPyramidRGBD> &currFrame,
                               const Eigen::Matrix3f &R,
                               const Eigen::Vector3f &T, ResidualInfo &resInfo,
  ...
  std::shared_ptr<Eigen::Vector4f> distGradients =
  ...
  I3D_LOG(i3d::detail) << std::fixed << "goodCount: " << (resInfo.goodPtsEdges)
...
float Optimizer::trackFrames(const std::shared_ptr<ImgPyramidRGBD> &refFrame,
  ...
  I3D_LOG(i3d::trace) << "Optimizer trackFrames:\nR:\n" << R << "\nt:\n" << T;
  // ============ track frame ============
  Eigen::Quaterniond q(R.cast<double>());
  q.normalize();
  Sophus::SE3d referenceToFrame(q.toRotationMatrix(), T.cast<double>());
  lsd_slam::LGS6 ls;
  double lastErr = calcErrorAndBuffers(refFrame, currFrame, R, T, resInfo, lvl);
  double last_residual = lastErr;
  double LM_lambda = mSettings.lambdaInitial[lvl];
  ...
  for (int iteration = 0; iteration < mSettings.maxItsPerLvl[lvl];
    ...
    while (true) {
      ...
      if (!inc.allFinite()) {
        I3D_LOG(i3d::error) << "Increment vector contains NaN or Inf values!";
        return last_residual;
      }
      Sophus::SE3d new_referenceToFrame =
          Sophus::SE3d::exp(inc) * referenceToFrame;
      // re-evaluate residual
      double error = calcErrorAndBuffers(
          refFrame, currFrame,
          new_referenceToFrame.rotationMatrix()
              .cast<float>() /*.cast<float>()*/,
          new_referenceToFrame.translation().cast<float>() /*.cast<float>()*/,
          resInfo, lvl);
  ...
  R = referenceToFrame.rotationMatrix().cast<float>(); //.cast<float>();
  T = referenceToFrame.translation().cast<float>();    //.cast<float>();
  return last_residual;
}
