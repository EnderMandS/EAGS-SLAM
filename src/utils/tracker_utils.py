...
def compute_camera_opt_params(estimate_rel_w2c: np.ndarray, device:int=0) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    opt_cam_rot = torch.nn.Parameter(np2torch(quaternion, device))
    opt_cam_trans = torch.nn.Parameter(np2torch(estimate_rel_w2c[:3, 3], device))
