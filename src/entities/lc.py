...
class Loop_closure(object):
    def __init__(self, config: dict, dataset: BaseDataset, logger: Logger) -> None:
        ...
        self.futures.append(self.executor.submit(self.load_netvlad))
        self.submap_lc_info = dict()
        self.submap_id = 0
        self.output_path = Path(config["data"]["output_path"])
        self.submap_path:Path = None # self.output_path / "submaps"
        self.pgo_count = 0
        self.n_loop_edges = 0
        self.opt = OptimizationParams(ArgumentParser(description="Training script parameters"))
        self.proj_matrix = getProjectionMatrix2(znear=0.01, zfar=100.0,
                fx = self.config["cam"]["fx"], fy = self.config["cam"]["fx"],
                cx = self.config["cam"]["cx"], cy = self.config["cam"]["cy"],
                W = self.config["cam"]["W"], H = self.config["cam"]["H"],
                device=self.device
            ).T
        ...
        self.PARALLEL:bool = self.config["lc"]["parallel"] if "parallel" in self.config["lc"] else True
        ...
        self.Twc = torch.empty(len(self.dataset), 4, 4, dtype=torch.float64, device=self.device)

    def load_netvlad(self):
        try:
            self.netvlad = GlobalDesc(self.device)
        except Exception as e:
            print(f"Error in init netvlad")
            print(traceback.format_exc())
            raise
    
    def update_submaps_info_from_file(self, id:int) -> None:
        """
        Update the submaps_info from submap file 
        Args:
            id (int): The submap id
        """
        submap_desc = []
        with torch.no_grad():
            submap = Submap().load(self.submap_path/f"{str(id).zfill(6)}.ckpt", device=self.device)
            if submap.kf_id[0]==0:
                self.Twc[:submap.Tmc.shape[0]] = submap.T_prev_m @ submap.Tmc
            else:
                Twm = self.Twc[submap.kf_id[0]-1] @ submap.T_prev_m
                self.Twc[submap.kf_id[0]:submap.kf_id[0]+submap.Tmc.shape[0]] = Twm @ submap.Tmc
            np_kf_ids = submap.kf_id.detach().cpu().numpy()
            for kf_id in np_kf_ids:
                submap_desc.append(self.netvlad(np2torch(self.dataset[kf_id][1],
                                                         self.device).permute(2, 0, 1)[None]/255.0))
            submap_desc = torch.cat(submap_desc).to(self.device)
    ...
    def submap_loader(self, id:int):
        ...
        """
        submap = Submap().load(self.submap_path / f"{str(id).zfill(6)}.ckpt",
                               device=self.device, verbose=self.VERBOSE)

        submap_cams = []
        np_kf_ids = submap.kf_id.clone().detach().cpu().numpy()
        for kf_id in np_kf_ids:
            _, _, depth, c2w_gt = self.dataset[kf_id]
            T_gt = torch.from_numpy(c2w_gt).to(self.device).inverse()
            T_est = self.Twc[kf_id].inverse().float()
            cam_i = Camera(kf_id, None, None, T_gt, self.proj_matrix, 
                   self.config["cam"]["fx"], self.config["cam"]["fx"],
                   self.config["cam"]["cx"], self.config["cam"]["cy"],
                   self.fovx, self.fovy, 
                   self.config["cam"]["H"], self.config["cam"]["W"], device=self.device)
            cam_i.R = T_est[:3, :3]
            cam_i.T = T_est[:3, 3]
        ...
        if id == 0:
            ...
            T_prev = self.Twc[submap.kf_id[0]-1]
    ...
    def detect_closure(self, query_id:int):
        ...
        db_info_list = [self.submap_lc_info[i] for i in range(query_id)]
        db_desc_map_id = []
        for db_info in db_info_list:
            db_desc_map_id += [db_info['submap_id'] for _ in db_info['kf_desc']]
        db_desc_map_id = torch.tensor(db_desc_map_id, dtype=torch.int, device=self.device)
    ...
    def construct_pose_graph(self, id:int):
        """
        ...
        for source_id in reversed(range(1, n_submaps)):
            ...
            if source_id == id and not new_submap_valid_loop:
                break
                
        torch.cuda.empty_cache()
        return pose_graph, odometry_edges, loop_edges
    
    def loop_closure(self, id:int) -> list:
        '''
        ...
        if id<3 or len(self.detect_closure(id)) == 0:
            if self.config["verbose"]:
                print(f"\nNo loop closure detected at submap no.{id:d}")
            return []
        
        pose_graph, odometry_edges, loop_edges = self.construct_pose_graph(id)
        ...
        if len(loop_edges)>0 and len(loop_edges) > self.n_loop_edges: 
            ...
            for i in range(id+1):
                submap_correction = {
                    'submap_id': i,
                    "correct_tsfm": pose_graph.nodes[i].pose}
    ...
    def analyse_pgo(self, odometry_edges, loop_edges, pose_graph):
        ...
        for i, node in enumerate(pose_graph.nodes):
            for kf_id in self.submap_lc_info[i]['kf_id']:
                ...
                updated_RT = (cam.get_T @ torch.from_numpy(node.pose).float().to(device=self.device).inverse())
    ...
    def pairwise_registration(self, submap_source, submap_target, method="gs_reg"):
        ...
        if method == "gt":
            ...
            res = gs_reg(submap_source, submap_target, self.config['lc']['registration'], device=self.device)
    ...
    def apply_correction_to_submaps(self, correction_list:dict) -> None:
        '''
        Apply correction to submaps

        Args:
            correction_list (dict): The correction_list dictionary from loop closure
        '''
        submap_dicts = {}
        ...
        for correction in correction_list:
            ...
            file_name = self.submap_path / Path(f"{str(submap_id).zfill(6)}.ckpt")
            submap_dict = torch.load(file_name, map_location=torch.device(self.device))
            submap_dicts[submap_id] = submap_dict
            tsfm_matrix = torch.from_numpy(np.copy(correct_tsfm)).to(dtype=torch.float64, device=self.device)
            start = submap_dict["kf_id"][0]
            end = start + submap_dict["Tmc"].shape[0]
            self.Twc[start:end] = tsfm_matrix @ self.Twc[start:end]

        # Apply to T_prev_m and save
        for correction in correction_list:
            submap_id = correction['submap_id']
            submap_dict = submap_dicts[submap_id]
            i = submap_dict["kf_id"][0]
            if i!=0:
                submap_dict["T_prev_m"] = self.Twc[i-1].inverse() @ self.Twc[i]
            torch.save(submap_dict, self.submap_path / Path(f"{str(submap_id).zfill(6)}.ckpt"))

    def save_Twc(self, path:str|Path="", start:int=0, end:int=0) -> None:
        '''
        Save camera poses under world coordinate to file.

        Args:
            path (str|Path): The path to save
            start (int): The start index to save of Twc.
            end (int): The end index to save of Twc. Set to 0 will save [start:]
        '''
        if path=="":
            path = self.output_path
        if end==0:
            save_dict_to_ckpt(self.Twc[start:].float(), "estimated_c2w.ckpt", directory=path)
        else:
            save_dict_to_ckpt(self.Twc[start:end].float(), "estimated_c2w.ckpt", directory=path)

    def run(self, id:int, frame_id:int) -> None:
        '''
        The loop closure thread

        Args:
            id (int): The submap id
            frame_id (int): The current frame_id. It should be the frame_id of the last frame in the submap
        '''
        try:
            if self.VERBOSE:
                print(f"LC {id:d} start.")
            torch.cuda.set_device(self.device)
            self.update_submaps_info_from_file(id)
            lc_output = self.loop_closure(id)
            if len(lc_output) > 0:
                self.apply_correction_to_submaps(lc_output)
            self.save_Twc(end=frame_id)
            torch.cuda.empty_cache()
            if self.VERBOSE:
                print(f"LC {id:d} complete.")
        except Exception as e:
            print(f"Error in LC {id:d}: {e}")
            print(traceback.format_exc())
            raise

    def submit(self, id:int, frame_id:int) -> concurrent.futures.Future:
        '''
        Submit a loop closure task to concurrent executor
        ...
        Returns:
            The future return of executor.submit()
        '''
        if self.VERBOSE:
            print(f"Submit LC {id:d} task")
        future = self.executor.submit(self.run, id, frame_id)
        self.futures.append(future)
        return future

    def check_futures(self) -> None:
        '''
        Check the concurrent executor threads result, raise exception if sub-thread exit with exception rasing
        '''
