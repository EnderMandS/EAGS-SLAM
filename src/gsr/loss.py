...
def image_gradient(image, device:int=0):
...
def image_gradient_mask(image, eps=0.01, device:int=0):
...
def depth_reg(depth, gt_image, huber_eps=0.1, mask=None, device:int=0):
    mask_v, mask_h = image_gradient_mask(depth, device=device)
    gray_grad_v, gray_grad_h = image_gradient(gt_image.mean(dim=0, keepdim=True), device=device)
    depth_grad_v, depth_grad_h = image_gradient(depth, device=device)
