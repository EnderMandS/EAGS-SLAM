...
class Camera(nn.Module):
    def __init__(
        ...
        T = torch.eye(4, dtype=torch.float, device=device)
        self.R = T[:3, :3]
        self.T = T[:3, 3]
        self.R_gt = gt_T[:3, :3]
        self.T_gt = gt_T[:3, 3]
    ...
    def compute_grad_mask(self, config):
        ...
        gray_grad_v, gray_grad_h = image_gradient(gray_img, device=self.device)
        mask_v, mask_h = image_gradient_mask(gray_img, device=self.device)
    ...
    def load_rgb(self, image=None):
        ...
        if image==None and hasattr(self, "rgb_path"):
            self.original_image = torch.from_numpy(np.array(Image.open(self.rgb_path))).permute(2, 0, 1).to(self.device).float() / 255.0
