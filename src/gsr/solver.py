...
def viewpoint_localizer(viewpoint, gaussians, base_lr: float=1e-3, device:int=0):
    ...
    opt_params = []
    pipe = CustomPipeline()
    bg_color = torch.tensor([0, 0, 0], dtype=torch.float32, device=device, requires_grad=False)
    config = {
        'Training': {
            'monocular': False,
    ...
    for tracking_itr in range(opt_iterations):
        optimizer.zero_grad()
        render_pkg = render(
            viewpoint, gaussians, pipe, bg_color, device=device
        )
        image, depth, opacity = (
            render_pkg["render"],
            render_pkg["depth"],
...
def gaussian_registration(src_dict, tgt_dict, config: dict, visualize=False, device:int=0):
    """_summary_
    ...
    init_overlap = compute_overlap_gaussians(src_dict['gaussians'], tgt_dict['gaussians'], 0.1, device=device)
    if init_overlap< 0.2:
        return {
            'successful': False,
            "pred_tsfm": torch.eye(4, device=device),
            "gt_tsfm": torch.eye(4, device=device),
    ...
    for viewpoint in src_view_list:
        ...
        if config["use_render"]:
            render_pkg = render(viewpoint, src_3dgs, pipe, bg_color, device=device)
            viewpoint.load_rgb(render_pkg['render'].detach())
            viewpoint.depth = render_pkg['depth'].squeeze().detach().cpu().numpy()
        else:
            viewpoint.load_rgb()
        converged, pred_tsfm, residual, loss_log = viewpoint_localizer(viewpoint, tgt_3dgs, config["base_lr"], device=device)
    ...
    for viewpoint in tgt_view_list:
        if config["use_render"]:
            render_pkg = render(viewpoint, tgt_3dgs, pipe, bg_color, device=device)
            viewpoint.load_rgb(render_pkg['render'].detach())
            viewpoint.depth = render_pkg['depth'].squeeze().detach().cpu().numpy()
        else:
            viewpoint.load_rgb()
        converged, pred_tsfm, residual, loss_log = viewpoint_localizer(viewpoint, src_3dgs, config["base_lr"], device=device)
    ...
    residuals = torch.tensor(residual_list, dtype=torch.float, device=device)
    ...
    best_tsfm = torch.eye(4, dtype=torch.float, device=device)
    best_tsfm[:3,:3]  = R_w
    best_tsfm[:3, 3]  = t_w
