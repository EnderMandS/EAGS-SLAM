...
def check_proj(points, W, H, fx, fy, cx, cy, c2w, device:int=0):
    """
    ...
    c2w[:3, 1] *= -1.0
    c2w[:3, 2] *= -1.0
    points = torch.from_numpy(points).to(device=device).clone()
    w2c = np.linalg.inv(c2w)
    w2c = torch.from_numpy(w2c).to(device=device).float()
    K = torch.from_numpy(
        np.array([[fx, 0.0, cx], [0.0, fy, cy], [0.0, 0.0, 1.0]]).reshape(3, 3)
    ).to(device=device)
    ones = torch.ones_like(points[:, 0], device=device).reshape(-1, 1)
    homo_points = (
        torch.cat([points, ones], dim=1).reshape(-1, 4, 1).to(device=device).float()
    )  # (N, 4)
...
def calc_2d_metric(
    rec_meshfile, gt_meshfile, unseen_gt_pointcloud_file, align=True, n_imgs=1000, device:int=0
):
    """
    ...
    for i in tqdm(range(n_imgs)):
        while True:
            ...
            seen = check_proj(pc_unseen, W, H, fx, fy, cx, cy, c2w, device=device)
            if ~seen:
                break
...
def evaluate_reconstruction(
    ...
    try:
        result_2d = calc_2d_metric(str(mesh_path), str(gt_mesh_path), str(unseen_pc_path), align=True, n_imgs=1000, device=device)
