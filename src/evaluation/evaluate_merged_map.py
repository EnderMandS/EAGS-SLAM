...
class RenderFrames(Dataset):
    ...
    def __init__(self, dataset, render_poses: np.ndarray, height: int, width: int, fx: float, fy: float, exposures_ab=None, device:int=0):
    ...
    def __getitem__(self, idx):
        ...
        frame = {
            ...
            "render_settings": get_render_settings(
                self.width, self.height, self.dataset.intrinsics, estimate_w2c, device=self.device),
            "estimate_w2c": estimate_w2c
        }
...
def refine_global_map(pt_cloud: o3d.geometry.PointCloud, training_frames:list, max_iterations:int, config:dict,
    ...
    gaussian_model = GaussianModel(3, verbose=True)
    ...
    iteration = 0
    for iteration in tqdm(range(max_iterations), desc="Refinement", unit="iter"):
    ...
    try:
        if export_refine_mesh:
            ...
            for i in tqdm(range(len_frames), desc="Integrating mesh", unit="frame"):  # one cycle
                training_frame = next(training_frames)
                render_settings, estimate_w2c = training_frame["render_settings"], training_frame["estimate_w2c"]
                estimate_w2c = np.array(estimate_w2c, dtype=np.float64).squeeze()
                ...
                rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
                    ...
                    depth_scale=scale, depth_trunc=30,
                    convert_rgb_to_intensity=False)

                volume.integrate(rgbd, o3d_intrinsic, estimate_w2c)
